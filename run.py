...
def parse_arguments():
    ...
    parser.add_argument("--test_data_file", help="path to dataset"),
...
def train(args):
    ...
    if args.do_test:
        test, _ = read_data(args.test_data_file)
        test_dataset = NERDataset(test, labels_to_id, tokenizer, args.max_length)
        ...
        for test_batch in tqdm(test_dataloader):
            with torch.no_grad():
                tokens = test_batch.pop("tokens")
                test_batch = {key: test_batch[key].to(args.device) for key in test_batch}
                out = model.forward(**test_batch)
                loss = out.loss
                logits = out.logits
                logits = logits.argmax(dim=2).cpu().tolist()

                out = [[id_to_labels[int(logits[i][j])]
                        for j in range(len(logits[i])) if tokens[i][j] == 1]
                       for i in range(len(logits))]
                final_out += out

        pred_path = os.path.join(args.output_dir, "prediction")
        if not os.path.exists(pred_path):
            os.mkdir(pred_path)

        with open(os.path.join(pred_path, "test_prediction.json"), "w") as fl:
            json.dump(final_out, fl)
